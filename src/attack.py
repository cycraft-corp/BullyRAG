...
class PreferenceAttack(BaseAttack):
    ...
    def _rewrite_passage(self, model, passage): # is it good to put this function here as it doesn't require self?
    ...
    def _generate_summary(self, model, passage):
    ...
    def _inference(self, question, ai_passage, passage):
    ...
    def attack(self) -> None:
        for i in range(len(self.dataset.qa_pairs)):
            ...
            summary = self._generate_summary(self.model, passage)
            ai_passage_list = []
            ai_choices_list = []
            ai_answers_list = []
            # summaries_list = []
            for question in questions:
                ai_passage = self._generate_passage(self.model, question, passage)
                chosen_passage, answer = self._inference(self.model, question, ai_passage, passage)
